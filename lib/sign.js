...
Sign.doSign = function(req)
{
    ...
    if (advNonce.length != 32) {
        return Server.marshalResponse(ResProps, {
            okay : false,
            error: util.format('nonce has incorrect length of %d, must be 16 bytes.', advNonce.length / 2)
        });
    }
    ...
    // nymi band wants concatenation of raw r and s values as hexadecimal string
    signature = Buffer.concat([signature.r.toBuffer(), signature.s.toBuffer()]).toString('hex');
